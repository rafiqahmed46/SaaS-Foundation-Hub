...
import { getTechnician, updateTechnician, getTasks, Technician, Task } from "@/lib/firestore";
...
import { ArrowLeft, Mail, Phone, Wrench, Pencil, CheckCircle2, Clock, Circle, AlertTriangle, User } from "lucide-react";
...
export default function TechnicianDetailPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", specialization: "", status: "active" as "active" | "inactive", notes: "" });
  const [saving, setSaving] = useState(false);

  async function load() {
    if (!user?.companyId || !params.id) return;
    try {
      const [tech, allTasks] = await Promise.all([getTechnician(params.id), getTasks(user.companyId)]);
      setTechnician(tech);
      setTasks(allTasks.filter((t) => t.assignedTo === params.id));
  ...
  function openEdit() {
    ...
    setForm({ name: technician.name, email: technician.email, phone: technician.phone || "", specialization: technician.specialization || "", status: technician.status, notes: technician.notes || "" });
    setEditOpen(true);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const upd = { name: form.name.trim(), email: form.email.trim(), phone: form.phone.trim() || undefined, specialization: form.specialization || undefined, status: form.status, notes: form.notes.trim() || undefined };
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-4">
                  {technician.email && <a href={`mailto:${technician.email}`} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><Mail className="w-4 h-4 shrink-0 text-primary/60" />{technician.email}</a>}
                  {technician.phone && <a href={`tel:${technician.phone}`} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><Phone className="w-4 h-4 shrink-0 text-primary/60" />{technician.phone}</a>}
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
                ...
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Phone</p><p className="font-medium">{technician.phone || "—"}</p></div>
          ...
          <div className="space-y-2">
            {tasks.length === 0 ? (
              <div className="rounded-xl border border-dashed p-10 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Assign tasks to this technician from the Tasks page.</p>
              </div>
            ) : tasks.map((task) => {
              const overdue = isOverdue(task);
              const sc = STATUS_CFG[task.status] ?? STATUS_CFG.todo;
              return (
                <div key={task.id} className={cn("rounded-xl border bg-card p-4", overdue && "border-red-200 bg-red-50/30")}>
